...
{
  int val = (int)arg;
  printf(1, "SBRK Thread %d start\n", val);

  int i, j;

  if (val == 0) {
    ptr = (int *)malloc(65536);
    printf(1, "[SBRK TEST] %d\n\n", val);
    sleep(100);
    free(ptr);
    ptr = 0;
  }
...
int main(int argc, char *argv[])
{
  ...
  printf(1, "[RESULT] Test 2 passed\n\n");

  printf(1, "Test 3: Sbrk test\n");
  create_all(NUM_THREAD, thread_sbrk);
  join_all(NUM_THREAD);
  printf(1, "[RESULT] Test 3 passed\n\n");
